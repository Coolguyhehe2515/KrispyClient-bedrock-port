...
static void renderFrame()
{
    ...
    if (g_window != nullptr)
    {
        int width = ANativeWindow_getWidth(
            g_window
        );

        int height = ANativeWindow_getHeight(
            g_window
        );
    ...
    {
        LOGE(
            "eglMakeCurrent render failed: 0x%04x",
            eglGetError()
        );
